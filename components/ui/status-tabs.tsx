...
const statusTabItemVariants = cva(
  "flex-1 px-2.5 py-2.5 inline-flex flex-col justify-center items-center text-xs transition-all duration-150",
  {
    variants: {
      variant: {
        default: "border-y",
...
interface StatusTabsProps
  extends
    ...
    VariantProps<typeof statusTabsVariants> {
  options: StatusTabOption[];
  activeValue: string;
  onChange: (value: string) => void;
}

const StatusTabs = React.forwardRef<HTMLDivElement, StatusTabsProps>(
  (
    {
      options,
      activeValue,
